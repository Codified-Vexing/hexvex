# Version 0.1.0 - 14/2/2019
# License: GNU GPLv3
# Author: Diogo Duarte
# This is the most basic assembler. No clever tricks and very straightforward mnemonic-to-hex conversion.
...
for addr in range(2**16):
	if addr in fakeROM:
		if addr%4 == 0:
			print("-"*42,"[",instr, "]", sep="")
			instr += 1
		print(addr, "\t|\t", fakeROM[addr], "\t|\t", hex(fakeROM[addr]))	

print("\n---Falstad's Format---")
print("Inputs: ", *inps, sep=", ")
print("Outputs: ", *outs, sep=", ")
for var, val in fakeROM.items():
	print(bin(var)[2:].zfill(len(inps)), "=", bin(val)[2:].zfill(len(outs)))
...
for addr in range(2**16):
	if addr in fakeROM:
		print(hex(fakeROM[addr])[2:].zfill(2),end=" ")
		tofile += hex(fakeROM[addr])[2:].zfill(2)+" "
		cnt+=1
	else:
		tofile += "00 "
print("\nNumber of hexes:", cnt)
with open(prog_dir[:-4]+"rom", "w") as f:
			f.write(tofile)
print("\n***A file with all ROM data was written!***")
